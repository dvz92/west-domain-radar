...
大池子里没被抽到的域名是**站点限制**，不是漏跑。想保证某个域名一定出现，加进
`assets/watchlist.txt`：每天会直查它（`domkey` 是精确匹配）并列出真实删除日期与状态。
...
- `assets/watchlist.txt` —— 你点名的域名，每天会单独核查状态与**本轮排名**；
  只有真实删除日期等于该后缀本批次日期的，才会并进候选池参与排序（不属于的只列出、标注 📌）
