...
报告第三节末尾有一行**四声母诊断**：本批次 4 位域名多少个、命中好词表多少个。

- 四声母**只可能来自 `.cn/.top` 的 ≤4 位池** —— `.com` 从 5 位起，4 位 com 不在口径内
- 好词表（`assets/good4.txt`）目前是 **240 个四字词 / 210 个首字母组合**，
  而 4 位组合一共有 26⁴ = 45.7 万种 → 命中本来就稀疏
- 2026-09-26 实测：当天 .cn 的 4 位池抽样 100 个，其中 **79 个是"读不出来的串"**
  （正是四声母的形态），但**一条都没落在好词表里** → 所以输出 0 是"真没有"，
  不是"没查到"。而 09-25 那天就抓到过 `xrmm`（笑容满面）和 `wcsz`（物超所值），
  说明机制本身是通的
- 想提高命中率：往 `assets/good4.txt` 里加你认可的四字词（一行多个，Tab 分隔）
...
`smtplib` 直发，附件是 HTML + Markdown，正文是 Top10 摘要 + 竞争提示。
